...
from benefactors.forms import LoginForm, SignUpForm, AccountUpdateForm, DonationForm, PostForm, RequestResetForm, \
    ResetPasswordForm, SearchForm, PostCommentForm, SendMessageForm, ReviewForm, DeliveryRequestForm
...
# -------------------------------------------------ENSC 440 DPL---------------------------------------------------------
@app.route("/upload/<int:delivery_id>", methods=['GET', 'Post'])
def upload(delivery_id):
    file = request.files['image']
    img = Image.open(file.stream)
    rgb_im = img.convert('RGB')
    rgb_im.save(f'benefactors/static/pickupImages/{delivery_id}.jpg')
...
def deliveryIdSee(delivery_id):
    ...
    return render_template('DeliverySee.html', delivery=delivery)
...
def getDelivery(_passCode):
    _passCode = int(_passCode)
    targetDelivery = Deliveries.query.filter_by(tracking_number=_passCode).first()
    if not targetDelivery:
        return {}
    return {'delivery_id': targetDelivery.delivery_id,
...
def create_new_post():
    ...
    if form.validate_on_submit():
        post = Deliveries(title=form.title.data, description=form.description.data, tracking_number = form.tracking_number.data,
                    passcode = "123123123", to_customer = current_user.id)
        db.session.add(post)
        db.session.commit()
        print(post.title)
